...
def RMSprop(cost, params, lr=0.001, decay=0.0001, rho=0.9, eps=1e-6):
	grads = T.grad(cost=cost, wrt=params)
	updates = []
	for p, g in zip(params, grads):
		r = theano.shared(p.get_value() * 0.0)
		r_new = rho * r + (1 - rho) * (g ** 2)
		updates.append([p, p - lr * (g / T.sqrt(r_new + eps)+ decay*p)])
...
updates = RMSprop(cost, params, lr=learning_rate, decay=decay, rho=rho, eps=eps)
...
pylab.savefig('figure_2a_3_test_accuracy.png')

pylab.figure()
pylab.plot(range(epochs), costs)
pylab.xlabel('epochs')
pylab.ylabel('training cost')
pylab.savefig('figure_2a_3_train_cost.png')
